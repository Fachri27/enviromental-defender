            ...
            <table class="min-w-full text-sm text-left text-gray-700">
                <thead class="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th class="px-4 py-3 font-medium text-gray-500">Link</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @php
                        $translation = $regulasi->translations->first();
                    @endphp
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 text-gray-600">{{ ($regulasis->currentPage() - 1) * $regulasis->perPage() + $index + 1 }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900">{{ $translation->title }}</td>
                        <td class="px-4 py-3 text-gray-700 capitalize truncate max-w-xs">{{ $regulasi->link }}</td>
                        <td class="px-4 py-3 text-right space-x-3">
                            <a href="{{ route('regulasi.edit', $regulasi->id) }}">
                                <button class="text-blue-600 hover:underline text-sm font-medium cursor-pointer">Edit</button>
                            </a>
                            <button wire:click='destroy({{ $regulasi->id }})' class="text-red-600 hover:underline text-sm font-medium ml-2 cursor-pointer">Delete</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
